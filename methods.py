...
def torch_CHROM(signal):
    ...
    """
    X = signal
    Xcomp = 3*X[:, 0] - 2*X[:, 1]                  #x component
    Ycomp = (1.5*X[:, 0])+X[:, 1]-(1.5*X[:, 2])    #y component
    sX = torch.std(Xcomp, axis=1)       #standard deviation along x-axis using torch
    sY = torch.std(Ycomp, axis=1)       #standard deviation along y-axis using torch
    alpha = (sX/sY).reshape(-1, 1)      #ratio of standard deviations in a column vector
    alpha = torch.repeat_interleave(alpha, Xcomp.shape[1], 1)       #alpha replicated along second axis
...
def cpu_LGI(signal):
    ...
    X = signal
    #print(X.shape)
    U, K, F = np.linalg.svd(X)      #singular value decomposition on the input RGB signal
    #print(U.shape)
    #print(K.shape)
    #print(F.shape)
    S = U[:, :, 0]          # taking most relevant data into a subspace S, maximum variance in left singular vectors in [U]
    S = np.expand_dims(S, 2)        #expanding dimension of S by 1
    sst = np.matmul(S, np.swapaxes(S, 1, 2))    #symmetric matrix sst, multiplicaton of S with its transpose
    p = np.tile(np.identity(3), (S.shape[0], 1, 1))
    P = p - sst         #projection matrix
    Y = np.matmul(P, X)     #matrix multiplication, projecting matrix into a lower-dimensional subspace, where most important varaince is captured
    bvp = Y[:, 1, :]        #extracting second raw in the second dimension
    return bvp




def cpu_LGI_TSVD(signal, **kargs):

    bvp = []
    for i in range(signal.shape[0]):        # which is 1 in this case due to the holistic approach..
        
        X = signal[i]
        if kargs['Stress'] == 'False':
            svd = TruncatedSVD(n_components=1, n_iter=1, random_state=42)  # TruncatedSVD method is fitted to the input signal, reducing dimensionality to 1
            svd.fit_transform(X)
            bvp.append(svd.components_[0]*svd.singular_values_[0])
        else:
            svd = TruncatedSVD(n_components=3, n_iter=1, random_state=42)  # TruncatedSVD method is fitted to the input signal, but all three components are extracted, as a full SVD
            svd.fit_transform(X)
            bvp.append(svd.components_[0]*svd.singular_values_[0])
            bvp.append(svd.components_[1]*svd.singular_values_[1])
            bvp.append(svd.components_[2]*svd.singular_values_[2])
...
def cpu_POS(signal, **kargs):
    ...
    P = np.array([[0, 1, -1], [-2, 1, 1]])  #projection matrix 
    Q = np.stack([P for _ in range(e)], axis=0) #4D matrix
    ...
    for n in np.arange(w, f):
        ...
        Cn = X[:, :, m:(n + 1)]     #third dimension is different each iteration, thanks to the sliding index m
        M = 1.0 / (np.mean(Cn, axis=2)+eps)     #normalization factor
        M = np.expand_dims(M, axis=2)  # shape [e, c, w]
        Cn = np.multiply(M, Cn)     #normalizing RGB frames..

        # Projection (6)
        S = np.dot(Q, Cn)   #project normalized data using 4D matrix Q --> S: 4D matrix
        S = S[0, :, :, :]
        ...
        # Overlap-adding (8)
        H[:, m:(n + 1)] = np.add(H[:, m:(n + 1)], Hnm)      #adding tuned values to output matrix H
...
def cpu_PBV(signal):
    ...
    """
    sig_mean = np.mean(signal, axis = 2)        #2D mean signal calculated along the 3rd axis

    signal_norm_r = signal[:,0,:] / np.expand_dims(sig_mean[:,0],axis=1)        #signal normalization..
    signal_norm_g = signal[:,1,:] / np.expand_dims(sig_mean[:,1],axis=1)
    signal_norm_b = signal[:,2,:] / np.expand_dims(sig_mean[:,2],axis=1)

    pbv_n = np.array([np.std(signal_norm_r, axis = 1), np.std(signal_norm_g, axis = 1), np.std(signal_norm_b, axis = 1)])   #numerator (pbv_n) is the standard deviation of each normalized signal across time
    pbv_d = np.sqrt(np.var(signal_norm_r, axis = 1) + np.var(signal_norm_g, axis = 1) + np.var(signal_norm_b, axis = 1))    #denominator (pbv_d) is the square root of the sum of variances of the normalized signals across time
    pbv = pbv_n / pbv_d     #ratio of standard deviation to variance

    C = np.swapaxes(np.array([signal_norm_r, signal_norm_g, signal_norm_b]),0,1)
    Ct =np.swapaxes(np.swapaxes(np.transpose(C),0,2),1,2)
    Q = np.matmul(C, Ct)
    W = np.linalg.solve(Q,np.swapaxes(pbv,0,1))     #linear system 'Q*W = pbv'

    A = np.matmul(Ct, np.expand_dims(W,axis = 2))
    B =  np.matmul(np.swapaxes(np.expand_dims(pbv.T,axis=2),1,2),np.expand_dims(W,axis = 2))
    bvp = A / B     #dim(bvp) = (number of frames) x (number of RGB channels)
    return bvp.squeeze(axis=2)      #last axis (axis=2) is squeezed to remove singleton dimensions and obtain the final BVP signal
...
def cpu_PCA(signal,**kargs):
    ...
    for i in range(signal.shape[0]):
        X = signal[i]
        pca = PCA(n_components=3)   #PCA method is fitted to the input signal, reducing dimensionality to 3
        pca.fit(X)
        ...
        if kargs['component']=='all_comp':
            ...
            bvp.append(pca.components_[1] * pca.explained_variance_[1])     #only second component as bvp extraction
    bvp = np.array(bvp)
    return bvp
...
def cpu_GRGB(signal):
    """
    GRGB method, proposed by Mohamed Elgendi, ETH Zürich.
    
    paper: "GRGB rPPG: An Efficient Low-Complexity Remote Photoplethysmography-Based Algorithm for Heart Rate Estimation"
    """
    G = signal[:,1,:]
    R = signal[:,0,:]
    B = signal[:,2,:]
...
def cpu_OMIT(signal):
    ...
    for i in range(signal.shape[0]):
        ...
        S = Q[:, 0].reshape(1, -1)      #extracting firts column
        P = np.identity(3) - np.matmul(S.T, S)  #projection matrix
        Y = np.dot(P, X)
        bvp.append(Y[1, :])
    bvp = np.array(bvp)
    return bvp
...
def cpu_ICA(signal, **kargs):
    ...
    bvp = []
    for X in signal:
        W = jadeR(X, verbose=False)  #W represents the matrix that linearly transforms the original signals into statistically independent components
    ...
    if kargs['component']=='all_comp':
        ...
        bvp = np.reshape(bvp[:,1,:], (l, f))        #only the second component is extracted
